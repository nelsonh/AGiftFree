...
@interface WavefrontObj (private)
	...
	if ((self = [super init]))
	{
		...
		for (NSString * line in lines)
		{
			...
			else if ([line hasPrefix:@"m"])
			{
				...
				NSString *mtlPath = [fileDirPath stringByAppendingPathComponent:truncLine];
				self.materials = [WavefrontMaterial materialsFromMtlFile:mtlPath];
			}
		...
		vertexCount = 0;
		faceCount = 0;
		WavefrontGroup *currentGroup = nil;
		...
		for (NSString * line in lines)
		{
			...
			else if ([line hasPrefix:@"g "])
			{
				...
				WavefrontMaterial *material = [materials objectForKey:materialName] ;
				if (material == nil)
					material = [WavefrontMaterial defaultMaterial];
				
				currentGroup = [[WavefrontGroup alloc] initWithName:groupName
			...
			else if ([line hasPrefix:@"f "])
			{
				...
				if (currentGroup == nil)
				{
					...
					WavefrontMaterial *tempMaterial = nil;
					...
					if (tempMaterial == nil)
						tempMaterial = [WavefrontMaterial defaultMaterial];
					 
					
					currentGroup = [[WavefrontGroup alloc] initWithName:@"default"
...
- (void)renderSelf
{
	...
	//glScalef(selfScale, selfScale, selfScale);
	
	
	glRotatef(rotXAngle, 1.0, 0.0, 0.0);
	glRotatef(rotYAngle, 0.0, 1.0, 0.0);
	glRotatef(rotZAngle, 0.0, 0.0, 1.0);
	...
	for (WavefrontGroup *group in groups)
	{
...
- (void)calculateNormals
{
	...
	{
		WavefrontGroup *oneGroup = [groups objectAtIndex:i];
